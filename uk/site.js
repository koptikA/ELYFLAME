...
const levels = [
  {level:1,label:'Перші кроки',ages:'3\u00a0роки',program:'Recreational',apparatus:'Скакалка',title:'Маленька іскра великого шляху.',text:'Знайомство з\u00a0рухом, координацією та\u00a0скакалкою в\u00a0грі.'},
  {level:2,label:'Свій ритм',ages:'4–5\u00a0років',program:'Recreational',apparatus:'Скакалка, м’яч',title:'Хай веде цікавість.',text:'Гра й\u00a0рух, дедалі більша впевненість і\u00a0перша дружба зі\u00a0скакалкою та\u00a0м’ячем.'},
  {level:3,label:'Основа',ages:'6–7\u00a0років',program:'Competitive',apparatus:'Скакалка, м’яч, обруч',title:'Починається новий розділ.',text:'Скакалка, м’яч і\u00a0обруч разом: гімнастки починають готуватися до\u00a0змагань.'},
  {level:4,label:'Сила',ages:'від\u00a06\u00a0років',program:'Competitive',apparatus:'+\u00a0булави',title:'Місце для нового виклику.',text:'Додаються булави, а\u00a0розтяжка й\u00a0робота з\u00a0предметами лишаються в\u00a0центрі уваги.'},
  {level:5,label:'Виразність',ages:'від\u00a07\u00a0років',program:'Competitive',apparatus:'+\u00a0стрічка (усі п’ять)',title:'Хай розкриється виразність.',text:'Стрічка доповнює набір предметів, поки зростають навички й\u00a0гнучкість.'},
  {level:6,label:'На\u00a0килим',ages:'від\u00a08\u00a0років',program:'Competitive',apparatus:'Усі предмети',title:'Амбіції\u00a0— на\u00a0килим.',text:'Шлях до\u00a0серйозної змагальної гімнастики, рівень визначає тренер.'}
];
// Sample trial times only; the academy sets the real ones (they are not published as a class schedule).
const programs = [
  {name:'Recreational',days:['Понеділок, 17:00','Середа, 17:00']},
  {name:'Competitive',days:['Вівторок, 18:00','Четвер, 18:00']},
  {name:'Stretching & Flexibility',days:['П’ятниця, 19:00']}
];
const age=document.querySelector('#age'),experience=document.querySelector('#experience'),path=document.querySelector('.path');
path.innerHTML='<span class="path-spark" aria-hidden="true">✦</span>'+levels.map(l=>`<div class="level" data-level="${l.level}"><span class="level-number">${l.level}</span><div class="level-description"><p>${l.label}</p><small>${l.ages}</small><small class="apparatus">${l.apparatus}</small></div></div>`).join('')+'<span class="assessment-stop" hidden>Почнемо з\u00a0оцінки тренера</span>';
...
function updateMatch(){
  ...
  document.querySelector('#match-label').textContent=assessment?'Старт / Оцінка тренера':`Можливий старт / Рівень\u00a0${selected.level}`;
  document.querySelector('#match-title').textContent=assessment?'У\u00a0кожного свій шлях.':selected.title;
  document.querySelector('#match-copy').textContent=assessment?"Вік\u00a0— лише частина картини. Розкажіть про досвід дитини, і\u00a0на\u00a0пробному занятті тренер допоможе підібрати рівень.":selected.text;
...
function populateDays(){
  ...
  day.replaceChildren(new Option('Оберіть час',''));
  (schedule?.days||[]).forEach(value=>day.add(new Option(`${value} [приклад]`,value)));
...
function errorFor(field){
  ...
  switch(field.id){
    case 'child-name':return value.length<2||value.length>50?"Введіть ім’я дитини: від\u00a02 до\u00a050 символів":'';
    case 'trial-age':return !value?"Введіть вік дитини":!Number.isInteger(Number(value))||Number(value)<3||Number(value)>99?"Введіть вік дитини цілим числом від\u00a03 до\u00a099":'';
    case 'trial-experience':return !['none','some','competitive'].includes(value)?"Оберіть досвід дитини в\u00a0гімнастиці":'';
    case 'program':return !programs.some(p=>p.name===value)?'Оберіть програму':'';
    case 'preferred-day':return !programs.find(p=>p.name===program.value)?.days.includes(value)?'Оберіть час пробного для цієї програми':'';
    case 'parent':return value.length<2||value.length>80?'Введіть ваше ім’я: від\u00a02 до\u00a080 символів':'';
    case 'phone':return value.replace(/\D/g,'').length!==10?'Введіть номер телефону в\u00a0США з\u00a010\u00a0цифр':'';
    case 'email':return !value||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)?'Введіть адресу ел. пошти, наприклад name@example.com':'';
    case 'consent':return !field.checked?'Позначте згоду на\u00a0зв’язок щодо пробного заняття':'';
    default:return '';
  }
}
...
function bookingRows(){
  return [['Дитина',form.elements.childName.value],['Вік',form.elements.age.value],['Досвід',form.elements.experience.selectedOptions[0].text],['Програма',program.selectedOptions[0].text],['Час',`${day.value} [приклад]`],['Оплата',form.elements.payment.value==='online'?'10\u00a0$ онлайн (у\u00a0прототипі тут буде Stripe Checkout)':'10\u00a0$ в\u00a0академії'],['Ваше ім’я',form.elements.parent.value],['Телефон',form.elements.phone.value],['Ел. пошта',form.elements.email.value]];
}
function resetLoading(){
  fields.forEach(field=>field.disabled=false);submitting=false;submit.disabled=false;form.removeAttribute('aria-busy');
  form.querySelector('.submit-label').textContent='Записатися';form.querySelector('.spinner').hidden=true;form.querySelector('.submit-arrow').hidden=false;
}
...
form.addEventListener('submit',async event=>{
  ...
  if(!summary.hidden){status.textContent='Перевірте позначені поля';if(dialog.open){dialog.scrollTop=0;summary.focus({preventScroll:true});}else summary.focus();return;}
  const version=++submissionVersion,rows=bookingRows();
  submitting=true;submit.disabled=true;form.setAttribute('aria-busy','true');
  form.querySelector('.submit-label').textContent='Надсилаємо…';form.querySelector('.spinner').hidden=false;form.querySelector('.submit-arrow').hidden=true;status.textContent='Готуємо підтвердження прототипу…';fields.forEach(field=>field.disabled=true);
  try{
  ...
  }catch{status.textContent='Не\u00a0вдалося надіслати. Дані збережено, спробуйте ще раз.';submit.classList.add('has-error');}
  finally{
