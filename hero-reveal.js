...
// Both libraries render static text when the visitor prefers reduced motion.
...
document.fonts.ready.then(() => {
  ...
  for (const line of lines) {
    ...
    setTimeout(() => { show(line); seregaGentle(line) }, delay)
    delay += line.textContent.length * 15 + 120
  }
  setTimeout(() => { show(accent); seregaEmotional(accent) }, 950)
