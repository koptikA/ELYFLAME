...
- `↗` marks external links; internal actions use `→`; in-page jumps (the `.page-index` strips) use `↓`. The page fonts lack `→`, `↗` and `✳`, so they come from the `Glyphs` subsets in `assets/fonts/` (in every font stack); iPhones draw a symbol missing from the stack as a color emoji, so add any new symbol to those subsets.
