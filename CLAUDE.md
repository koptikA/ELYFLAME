...
- Open questions: `docs/client-questions.md`. Cite them by number (D1–D5 for the PM, 1–29 for the client).
- Prototype: `index.html` (Home), `about/index.html` (About), shared `site.css` and `site.js` (English sources). `ru/` and `uk/` are generated by `tools/i18n.py`; register new page directories in `PAGES`. The generator checks copied headers/footers and same-page language links.
