...
- A light paper hero pairs the headline with the mirrored Home gymnast silhouette (`#gymnast-shape`, ink, no leg animation) and a short satin ribbon painted by `paintSatin()` inside a reserved illustration column. This combined brand composition is temporary until Alena supplies a stretching silhouette. No footnote or scrolling Ribbon; the ribbon stays clear of copy and the button. The silhouette and ribbon share one `.stretch-figure[data-illustration]` container; replace only this container when the new illustration arrives.
...
Stretching, Contact and Parents’ Info start on paper. Dark stage is reserved for the trial banner before the shared footer. Use existing silhouettes, paintSatin satin ribbon or a decor-colored ✳, at most one decorative composition per screen; no homemade line icons, arcs or oversized numbers. Column gaps are 40–88 px at desktop, with content aligned toward the start. Parents’ Info alternates a horizontal page index, FAQ columns, full-width registration/policy/kit rows and narrower document/private-lesson sections. Registration is light and has no decorative step numbers. FAQ and unavailable PDF behavior are unchanged. External-site links use ↗; tel/mailto links do not.
