...
- `/stretching/`, `/ru/stretching/`, `/uk/stretching/`: one weekly class, with five audience entries: dancers, figure skaters, martial artists, runners, and teens/adults. No square-bracket placeholders remain on the page. Benefits are three draft lines with no timelines or medical promises ("A wider, freer range of motion", "Better posture and body control", "Flexibility that supports your main sport"), waiting for question 32. The class block lists Frequency (once a week), Schedule ("Ask the academy for current class times"), Starting level ("The coach sets your load at the trial") and a draft four-step structure: warm-up and joint mobility, deep static stretching, splits and bridge work, cool-down and breathing (question 30). The class duration is not shown until the client gives it. The block ends with a Book a Trial button that carries the program; the old mailto link is gone. The benefits band keeps its cream background.
- The paper hero has text on the left and the wide front-split gymnast from `assets/gymnast/split.svg#split` on the right, in ink. She sits on a light mat, a `--sand` strip a little wider than her and drawn in the same SVG (viewBox `-160 0 2643 1340`, symbol pinned at 2323 × 1253), so she reads as grounded without a full-width line; the hero and the audience block are separated by spacing only. No ribbon, no mirroring, no animation. At 1440 the figure fills the right column; at 390 it sits below the button at the full content width. The subtitle names only the format, "Once a week"; the audience lives in the next block. The temporary Home gymnast and hero footnote are removed; there is no scrolling Ribbon.
...
- No public schedule (answer 9): the Schedule row tells visitors to ask the academy for times. No class or membership prices (answer 8 supersedes the Sep 24 decision). The confirmed $10 trial fee remains in the shared header (answer 3).
...
- Contact form: name, email, phone and message, with the same field treatment as the trial form. Name, email and message are required; phone is optional, with the placeholder "(555) 123-4567" (as in the trial form) and the 14 px hint "Optional, for a call back."; when filled, it takes 10–15 digits with common punctuation. Limits are 100/254/40/5000 characters. Errors appear beside fields with `aria-describedby` and `aria-invalid`; JS focuses the first invalid field. There is no prototype notice on the form. After a valid submit JS clears the fields and shows, and focuses, "Your message is in! The academy will reply within 24 hours." Like the trial form, the prototype confirms locally and sends no request, so it also works on static hosting (Netlify). Without JS the form posts to `/api/contact`; a hidden language field keeps the locale, and `tools/serve.py` returns the page in that language with inline errors or the same confirmation. The prototype stores and sends nothing. Production needs a real POST handler that delivers to the address from question 33. This supersedes the earlier no-form decision and restores SOW 3.4. Trial booking still uses the single Home form.
...
**Strips, one look on every page (Alena, Sep 25).** In-page link strips share `.page-index`: Home ("Find the right class"), About (five sections) and Parents’ Info (four sections). Hairlines above and below span the content width; labels are 16 px ink, each followed by a generated ↓ (the target is lower on the same page, and the arrow is hidden from screen readers), magenta on hover. They wrap on phones. Lists split by a star, the Home values strip and the Stretching audience list, use the same ✳ in `--decor` at 0.8 em; the values strip keeps its Cinzel labels on sand, the audience list its plain text.
