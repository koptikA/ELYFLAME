...
- The paper hero has text on the left and the wide front-split gymnast from `assets/gymnast/split.svg#split` on the right, in ink, with viewBox `0 0 2323 1253`. The symbol’s bottom rests on the hero’s lower edge, the floor line before the next section; it is neither mirrored nor animated. A short paintSatin ribbon arcs upward and right from the raised hands in reserved space, clear of copy and the button. At 1440 the silhouette fills the right column; at 390 it sits below the button at the full content width. Silhouette and ribbon share one `.stretch-figure[data-illustration="split"]` container. Without JS the silhouette remains visible without ribbon; reduced motion keeps the composition static. The temporary Home gymnast and hero footnote are removed; there is no scrolling Ribbon.
