...
STRETCHING_HTML = [('Stretching & Flexibility — ElyFlame Academy',
 ...
 ('>Starting level<', '>Уровень подготовки<', '>Рівень підготовки<'),
 ('The coach sets your load at the trial.',
  'Тренер подберёт нагрузку на~пробном занятии.',
  'Тренер підбере навантаження на~пробному занятті.'),
 ('>How the class works<', '>Как проходит занятие<', '>Як проходить заняття<'),
 ('>Warm-up and joint mobility<', '>Разминка и~суставная гимнастика<', '>Розминка й~суглобова гімнастика<'),
 ('>Deep static stretching<', '>Глубокая статическая растяжка<', '>Глибока статична розтяжка<'),
 ('>Splits and bridge work<', '>Работа над шпагатом и~мостиком<', '>Робота над шпагатом і~містком<'),
 ('>Cool-down and breathing<', '>Заминка и~дыхание<', '>Заминка й~дихання<'),
...
STRETCHING_HTML += [('Beyond one sport.', 'За~пределами одного спорта.', 'За~межами одного спорту.'),
 ...
 ('A wider, freer range of motion.', 'Шире и~свободнее амплитуда движений.', 'Ширша й~вільніша амплітуда рухів.'),
 ('Better posture and body control.', 'Лучше осанка и~контроль тела.', 'Краща постава й~контроль тіла.'),
 ('Flexibility that supports your main sport.', 'Гибкость, которая помогает в~основном спорте.', 'Гнучкість, що допомагає в~основному спорті.')]
...
CONTACT_HTML += [('Let’s talk.', 'На~связи.', 'На~зв’язку.'),
 ...
 ('Your message is in! The academy will reply within 24 hours.',
  'Сообщение отправлено! Академия ответит в~течение 24~часов.',
  'Повідомлення надіслано! Академія відповість протягом 24~годин.'),
 ('Optional. Add it if you’d like a call back.',
  'Необязательно. Укажите, если хотите, чтобы вам перезвонили.',
  'Необов’язково. Вкажіть, якщо хочете, щоб вам передзвонили.'),
...
def main():
    ...
    for lang, col in LANGS.items():
        for page, html_src in sources.items():
            ...
            html = html.replace('name="language" value="en"', f'name="language" value="{lang}"')
